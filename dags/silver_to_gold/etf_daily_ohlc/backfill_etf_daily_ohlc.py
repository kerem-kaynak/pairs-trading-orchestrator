...
@task
def backfill_etf_daily_ohlc_silver_to_gold_task():
    try:
        logger.info("Starting backfill_etf_daily_ohlc_silver_to_gold task")
        ...
        query = f"""
            ...
            FROM silver.etf_daily_ohlc edo LEFT JOIN silver.ticker_details td ON edo.ticker = td.ticker;
        """
        ...
        logger.error(f"An error occurred in backfill_etf_daily_ohlc_silver_to_gold task: {str(e)}", exc_info=True)
        raise
...
def backfill_etf_daily_ohlc_silver_to_gold():
    backfill_etf_daily_ohlc_silver_to_gold_task()

dag = backfill_etf_daily_ohlc_silver_to_gold()